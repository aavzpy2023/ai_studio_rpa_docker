...
from config import *
from helpers import save_debug_snapshot, clean_prompt_text, is_spinner_visible, check_for_rate_limit
...
def setup_agent_environment(agent_type: str = None, new_chat: bool = False):
    ...
    if agent_type:
        ...
        if agent_inst_exists:
            ...
            try:
                crear_btns = list(pyautogui.locateAllOnScreen(IMG_CREATE_INSTR, grayscale=True, confidence=0.8))
            except Exception:
                crear_btns = []
...
def execute_rpa_task(prompt_text):
    ...
    for i in range(15):
        ...
        if check_for_rate_limit():
            raise RateLimitReachedException("Se alcanzó el límite de cuota mientras se esperaba generación.")
    ...
    if not spinner_aparecio:
        ...
        while is_spinner_visible():
            ...
            if check_for_rate_limit():
                raise RateLimitReachedException("Se alcanzó el límite de cuota durante la generación.")
    ...
    while intentos_scroll < MAX_SCROLL:
        try:
            ...
            if barra_visible:
                ...
                menus = list(pyautogui.locateAllOnScreen(IMG_TOOL_MENU, grayscale=True, confidence=0.8))
                
                if len(menus) >= 2:
                    print(f"      => Visibles {len(menus)} menús. Seleccionando el de la barra (el segundo)...")
                    ...
                    try:
                        copy_md_loc = pyautogui.locateOnScreen(IMG_COPY_MD, grayscale=True, confidence=0.9)
                        if copy_md_loc:
                            ...
                            print("      [Aviso] Se abrió el menú pero no se vio la opción Markdown. Cerrando y subiendo...")
