...
import bot_core
from helpers import RateLimitReachedException
...
def clean_ai_studio_markdown(raw_text):
    ...
    for i, linea in enumerate(lineas):
        ...
        if linea_limpia == 'code' and (i + 4) < len(lineas):
            if (lineas[i+2].strip() == 'download' and 
                lineas[i+3].strip() == 'content_copy' and 
                lineas[i+4].strip() == 'expand_less'):
        ...
        if in_code_block and (linea_limpia.startswith('PART ') or linea_limpia.startswith('IMPLEMENTATION:')):
...
async def chat_completions(request: ChatRequest):
    ...
    async with bot_lock:
        try:
            ...
            if comando_limpio.startswith("/rpa_change "):
                ...
                if agent_type not in ["skdev", "skarch"]:
                    raise ValueError("Comando incompleto. Usa '/rpa_change skdev' o '/rpa_change skarch'")
            ...
            elif comando_limpio.startswith("/rpa_mode ") or comando_limpio.startswith("/rpa_new talk"):
                ...
                if comando_limpio.startswith("/rpa_mode ") and not agent_type:
                    raise ValueError("Comando incompleto. Usa '/rpa_mode skdev' o '/rpa_mode skarch'")
            ...
            if comando_limpio.startswith("/rpa_"):
                if request.stream:
                    async def command_stream_generator():
                        chunk = {"id": f"chatcmpl-cmd-{int(time.time())}", "object": "chat.completion.chunk", "created": int(time.time()), "model": request.model or "gemini-rpa", "choices": [{"index": 0, "delta": {"content": msg}, "finish_reason": None}]}
                        yield f"data: {json.dumps(chunk)}\n\n"
                        end_chunk = {"id": f"chatcmpl-cmd-{int(time.time())}", "object": "chat.completion.chunk", "created": int(time.time()), "model": request.model or "gemini-rpa", "choices": [{"index": 0, "delta": {}, "finish_reason": "stop"}]}
                        yield f"data: {json.dumps(end_chunk)}\n\n"
                        yield "data: [DONE]\n\n"
                    return StreamingResponse(command_stream_generator(), media_type="text/event-stream")
                else:
                    return {"id": f"chatcmpl-cmd-{int(time.time())}", "object": "chat.completion", "created": int(time.time()), "model": request.model or "gemini-rpa", "choices": [{"index": 0, "message": {"role": "assistant", "content": msg}, "finish_reason": "stop"}], "usage": {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}}
            # -----------------------------------------------------

            # Si no es un comando especial, continúa con el RPA normal...
            print(f"\n" + "="*50)
            
            print(f"📥 New request received (Model: {request.model or 'default'}, Stream: {request.stream})")
            ...
            if final_prompt == last_processed_prompt and last_generated_response != "":
                ...
                print(f"🖱️ Handing over control to the RPA engine...")
            ...
            if request.stream:
                async def stream_generator():
                    chunk = {
                        ...
                        "choices": [{"index": 0, "delta": {"content": ai_response}, "finish_reason": None}]
                    }
                    yield f"data: {json.dumps(chunk)}\n\n"
        ...
        # --- BLOQUE DE MANEJO DE EXCEPCIONES ---
        ...
        except RateLimitReachedException as e:
            print(f"⚠️ [API] Límite de cuota detectado en Google AI Studio.")
            return JSONResponse(
            ...
            return JSONResponse(
                status_code=400,
                content={"error": {"message": str(e), "type": "invalid_request_error", "code": "bad_request"}}
            )
